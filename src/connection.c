...
int robotraconteurlite_connection_message_receive(struct robotraconteurlite_connection* connection, struct robotraconteurlite_message_reader* message_reader)
{
    ...
    if(robotraconteurlite_buffer_init(&buffer, connection->recv_buffer, connection->recv_buffer_pos))
    {
        return ROBOTRACONTEURLITE_ERROR_INTERNAL_ERROR;
    }
    if(robotraconteurlite_message_reader_init(message_reader, connection->recv_buffer, 0, connection->recv_buffer_pos))
    {
        return ROBOTRACONTEURLITE_ERROR_INTERNAL_ERROR;
    }
...
int robotraconteurlite_connection_begin_send_message(struct robotraconteurlite_connection* connection, struct robotraconteurlite_message_writer* message_writer)
{
    ...
    struct robotraconteurlite_buffer buffer;
    if(robotraconteurlite_buffer_init(&buffer, connection->send_buffer, connection->send_buffer_len))
    {
        return ROBOTRACONTEURLITE_ERROR_INTERNAL_ERROR;
    }
    if(robotraconteurlite_message_writer_init(&message_writer, connection->send_buffer, 0, connection->send_buffer_pos))
    {
        return ROBOTRACONTEURLITE_ERROR_INTERNAL_ERROR;
    }
