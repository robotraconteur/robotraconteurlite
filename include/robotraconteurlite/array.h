...
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_init_scalar(
...
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_copy_ex(
...
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_vec_ex(
...
/* double */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_double_array(
...
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_double_array(
...
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_double(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos, double* dest);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_double(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos, double source);

/* single */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_single_array(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_array_single* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_single_array(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
    const struct robotraconteurlite_array_single* source, size_t source_pos, size_t source_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_single(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos, float* dest);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_single(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos, float source);

/* int8 */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_int8_array(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_array_int8* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_int8_array(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
    const struct robotraconteurlite_array_int8* source, size_t source_pos, size_t source_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_int8(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos, int8_t* dest);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_int8(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos, int8_t source);

/* uint8 */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_uint8_array(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_array_uint8* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_uint8_array(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
    const struct robotraconteurlite_array_uint8* source, size_t source_pos, size_t source_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_uint8(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos, uint8_t* dest);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_uint8(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos, uint8_t source);

/* int16 */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_int16_array(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_array_int16* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_int16_array(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
    const struct robotraconteurlite_array_int16* source, size_t source_pos, size_t source_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_int16(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos, int16_t* dest);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_int16(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos, int16_t source);

/* uint16 */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_uint16_array(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_array_uint16* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_uint16_array(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
    const struct robotraconteurlite_array_uint16* source, size_t source_pos, size_t source_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_uint16(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos, uint16_t* dest);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_uint16(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos, uint16_t source);

/* int32 */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_int32_array(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_array_int32* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_int32_array(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
    const struct robotraconteurlite_array_int32* source, size_t source_pos, size_t source_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_int32(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos, int32_t* dest);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_int32(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos, int32_t source);

/* uint32 */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_uint32_array(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_array_uint32* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_uint32_array(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
    const struct robotraconteurlite_array_uint32* source, size_t source_pos, size_t source_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_uint32(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos, uint32_t* dest);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_uint32(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos, uint32_t source);

/* int64 */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_int64_array(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_array_int64* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_int64_array(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
    const struct robotraconteurlite_array_int64* source, size_t source_pos, size_t source_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_int64(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos, int64_t* dest);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_int64(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos, int64_t source);

/* uint64 */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_uint64_array(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_array_uint64* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_uint64_array(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
    const struct robotraconteurlite_array_uint64* source, size_t source_pos, size_t source_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_uint64(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos, uint64_t* dest);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_uint64(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos, uint64_t source);

/* cdouble */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_cdouble_array(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_array_cdouble* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_cdouble_array(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
...
/* csingle */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_csingle_array(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_array_csingle* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_csingle_array(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
...
/* struct robotraconteurlite_bool */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_bool_array(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_array_bool* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_bool_array(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
...
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_bool(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos, struct robotraconteurlite_bool source);

/* string */
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_to_string(
    const struct robotraconteurlite_buffer_vec* source_buf, size_t source_buf_pos,
    const struct robotraconteurlite_string* dest, size_t dest_pos, size_t dest_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_buffer_vec_copy_from_string(
    const struct robotraconteurlite_buffer_vec* dest_buf, size_t dest_buf_pos,
    const struct robotraconteurlite_string* source, size_t source_pos, size_t source_count);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_string_cmp(
...
ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_string_copy_to(
    const struct robotraconteurlite_string* source, struct robotraconteurlite_string* dest);

ROBOTRACONTEURLITE_DECL robotraconteurlite_status robotraconteurlite_string_shallow_copy_to(
    const struct robotraconteurlite_string* source, struct robotraconteurlite_string* dest);
