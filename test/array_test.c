	...
	{		
		...
		for (buf_i = 0; buf_i < 2; buf_i++)
		{
			...
			for (i = 0; i < 8; i++)
			{
				...
				buf_vec->buffer_vec_cnt++;
				pos += c;
				if (pos >= (sizeof(a1) / *buf_vec_els[buf_i])) break;
			}
		}
		...
		assert_return_code(robotraconteurlite_buffer_vec_copy_vec_ex(&bufa_vec, bufa_vec_start, bufa_vec_el, count_bytes / bufa_vec_el, 
			&bufb_vec, bufb_vec_start, bufb_vec_el, count_bytes / bufb_vec_el),0);
...
void robotraconteurlite_buffer_copy_to_double_test(void** state)
{
	...
	for (i = 0; i < sizeof(a1) / sizeof(double); i++) a1[i] = (double)i;
	memset(a2, 0, sizeof(a2));
...
int main(void)
{
	const struct CMUnitTest tests[] = {
		cmocka_unit_test(robotraconteurlite_arraytest_buffer),
