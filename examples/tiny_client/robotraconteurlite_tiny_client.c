    ...
    while (now < end_time && state != TINY_CLIENT_STATE_ERROR && state != TINY_CLIENT_STATE_DONE)
    {
        ...
        switch (state)
        {
            case TINY_CLIENT_STATE_INIT:
            {
                ...
                ret = robotraconteurlite_client_send_empty_request(&request_data, ROBOTRACONTEURLITE_MESSAGEENTRYTYPE_PROPERTYGETREQ, "d1", NULL);
                if (ret != ROBOTRACONTEURLITE_ERROR_SUCCESS)
                {
                    if (ret == ROBOTRACONTEURLITE_ERROR_RETRY)
                    {
            ...
            case TINY_CLIENT_STATE_GET_D1_RECEIVED:
            {
                ...
                request_data.node = &node;
                request_data.connection = connection;
                ret = robotraconteurlite_client_begin_request(&request_data, ROBOTRACONTEURLITE_MESSAGEENTRYTYPE_PROPERTYSETREQ, "d1", NULL);
                ...
                if (robotraconteurlite_messageelement_writer_write_double(&request_data.element_writer, &element_name, d1_set_val))
                {
